...
public class GoogleSitemap : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        List<Topic> dt = Util.GetTopicsForSiteMap(100);
        foreach (Topic topic in dt)
        {
            int topicId = topic.Id;

            var thisUrl = new Url(new Uri(string.Format(CultureInfo.InvariantCulture, "{0}/Content/Forums/topic.aspx?TOPIC_ID={1}", forumPage.Location.TrimEnd('/'), topicId)), topic.LastPostDate, ChangeFrequency.Daily, 0.5M);
