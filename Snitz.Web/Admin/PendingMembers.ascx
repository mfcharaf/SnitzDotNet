...
DataKeyNames="Id" EmptyDataText="No pending registrations" AllowPaging="True">
    <Columns>
        ...
        <asp:TemplateField HeaderText="Username/Email">
            <ItemTemplate>
                <asp:HyperLink ID="hypProf" runat="server" NavigateUrl='<%# Bind("ProfileLink") %>' Text='<%# Bind("Name") %>'></asp:HyperLink>
                <br />
        ...
        <asp:TemplateField HeaderText="Date">
            <ItemTemplate>
                <asp:Label ID="Label2" runat="server" Text='<%# Eval("Date").ToString().ToDateTime() %>'></asp:Label>
            </ItemTemplate>
            <HeaderStyle Width="80px" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="Email Validated">
            <EditItemTemplate>
                <asp:TextBox ID="email" runat="server" Text='<%# Eval("EmailValided") %>'></asp:TextBox>
            </EditItemTemplate>
            <ItemTemplate>
                <asp:Label ID="Lemail" runat="server" Text='<%# (Eval("EmailValided").ToString() == "1") ? "Yes" : "No" %>'></asp:Label>
            </ItemTemplate>
        ...
        <asp:TemplateField HeaderText="IP">
            <ItemTemplate>
                <asp:Label ID="Label1" runat="server" Text='<%# Bind("M_IP") %>'></asp:Label><br />
                <asp:HyperLink ID="spamcheck" runat="server" Text="Spam Check" NavigateUrl='<%# string.Format("http://www.stopforumspam.com/ipcheck/{0}",Eval("M_IP")) %>'></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField>
            <ItemTemplate>
                <asp:CheckBox ID="chkSelect" runat="server" ToolTip='<%# Eval("Name") %>' />
            </ItemTemplate>
...
<asp:ObjectDataSource ID="ObjectDataSourceMembershipUser" runat="server" 
    SelectMethod="GetPendingMembers" TypeName="SnitzData.PagedObjects" 
    OldValuesParameterFormatString="original_{0}" 
    EnablePaging="True"
    MaximumRowsParameterName="maxRecords" 
    StartRowIndexParameterName="startRecord" 
    SelectCountMethod="GetPendingCount">
