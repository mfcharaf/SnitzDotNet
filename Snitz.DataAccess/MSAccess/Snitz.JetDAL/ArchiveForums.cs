...
namespace Snitz.OLEDbDAL
{
    public class ArchiveForums : IArchiveForum
    {
        ...
        private const string TOPIC_COLS =
        ...
        public void ArchiveTopic(int forumid, int topicid)
        {
            ...
            updateSql.AppendFormat("").AppendLine("WHERE FORUM_ID = @ForumId ");
        ...
        public IEnumerable<TopicInfo> GetTopics(int startRowIndex, int maximumRows, int forumid, bool isAdminOrModerator)
        {
            ...
            over.AppendFormat("FROM {0}A_TOPICS", Config.ForumTablePrefix).AppendLine();
            over.AppendLine("ORDER BY T_LAST_POST DESC,T_SUBJECT");
            over.AppendLine(") sub");
            over.AppendFormat(") TE INNER JOIN {0}A_TOPICS T on TE.TOPIC_ID = T.TOPIC_ID ) ",Config.ForumTablePrefix).AppendLine();
            ...
            using (OleDbDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnString, CommandType.Text, "SELECT" + TOPIC_COLS + over.ToString() , param.ToArray()))
            {
                while (rdr.Read())
                {
        ...
        public TopicInfo GetTopic(int topicid)
        {
            ...
            using (var rdr = SqlHelper.ExecuteReader(SqlHelper.ConnString, CommandType.Text, "SELECT" + TOPIC_COLS + topicFrom, parm))
            {
                while (rdr.Read())
                {
