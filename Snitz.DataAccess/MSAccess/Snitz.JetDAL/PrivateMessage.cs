...
namespace Snitz.OLEDbDAL
{
    public class PrivateMessage : IPrivateMessage
    {
        ...
        public IEnumerable<MemberInfo> GetMemberListPaged(int page, string searchfor)
        {
            ...
            string SELECT_OVER = "WITH MemberEntities AS (SELECT ROW_NUMBER() OVER (ORDER BY M_NAME) AS Row, MEMBER_ID FROM " + Config.MemberTablePrefix + "MEMBERS LEFT OUTER JOIN " + Config.MemberTablePrefix + "ProfileData P ON P.UserId=" + Config.MemberTablePrefix + "MEMBERS.MEMBER_ID WHERE M_STATUS=1 AND IIF(PMEmail IS NULL,1,PMEmail)=1 ";
            if (!String.IsNullOrEmpty(searchfor))
            {
